...
YOLO_MODEL = os.getenv("YOLO_MODEL", "yolo11l-seg.pt") #Path to the YOLO model file since we are using the ultralytics YOLO api
...
INPUT_HEIGHT = int(os.getenv("INPUT_HEIGHT", 640)) 
...
# Dynamic counting lines will be generated based ROI_TUNER.py more of a dynamic approach than hardcoding
ROI_JSON = os.getenv("ROI_JSON", "shelf_rois.json") # Path to the JSON file containing shelf ROIs
SHELF_ROIS = [
    # Example 1: A main shelf area with a horizontal counting line in the middle
    # Taking out the example one, We will be using dynamic counting lines based on the shelf ROI
    #{"id": "main_shelf_area", "coords": [88, 102, 537, 296], "counting_line": [0, 150, 500, 150]},
    ...
    # Example 3: This is just the main shelf area, we will dynamically generate the counting line 
    # based on the shelf ROI
...
DB_CONFIG = {
    ...
    'user': os.getenv("DB_USER", "root"),# <<<<<<< IMPORTANT: CHANGE THIS IN PRODUCTION!
    'password': os.getenv("DB_PASSWORD", "root"), # <<<<<<< IMPORTANT: CHANGE THIS IN PRODUCTION!
